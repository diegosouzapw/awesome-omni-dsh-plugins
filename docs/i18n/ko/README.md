...
<img src="../../docs/assets/readme-hero.svg" width="100%" alt="Awesome Omni DSH Plugins — DeepSeek Harness 플러그인을 위한 크리에이터 우선 카탈로그와 원커맨드 설치. 고정된 소스 커밋, 공로를 인정받는 크리에이터, 플러그인당 하나씩 검토되는 PR. dsh-plugins.omniskill.online"/>
...
**DeepSeek Harness (DSH)** 플러그인을 위한 크리에이터 우선 발견과 원커맨드 설치.

<h2>
  🌐 <a href="https://dsh-plugins.omniskill.online"><strong>dsh-plugins.omniskill.online</strong></a> 🌐
</h2>
<h3>
  <a href="https://dsh-plugins.omniskill.online">웹사이트에서 모든 플러그인을 둘러보고, 검색하고, 설치하세요 →</a>
</h3>
...
<a href="README.md"><img src="../../docs/assets/flags/kr.svg" width="30" alt="한국어 (ko)" title="한국어 (ko)"></a>
...
정확한 저장소 스타 수를 기준으로 순위가 매겨집니다 — 부모 프로젝트의 스타는 절대 포함되지 않고, 오직 플러그인 자체 저장소가 획득한 스타만 집계됩니다([순위 산정 기준](../../docs/RANKING.md)). 모든 이름은 크리에이터의 저장소로 연결되며, 카탈로그가 검증한 정확한 커밋에 고정되어 있습니다.
...
### 👉 [**웹사이트에서 모든 플러그인을 검색하고, 세부 정보를 읽고, 설치 명령어를 복사하세요 →**](https://dsh-plugins.omniskill.online) 👈
...
| 영역     | 설명                                                       | 위치                                                                    |
| ----------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------ |
| **웹사이트** | 검색과 순위 기능을 갖춘 렌더링된 카탈로그 브라우저                 | [dsh-plugins.omniskill.online](https://dsh-plugins.omniskill.online)     |
...
| **머신 피드** | 도구를 위한 `catalog.json` + `catalog.snapshot.json`           | [catalog.json](https://dsh-plugins.omniskill.online/catalog.json) · [catalog.snapshot.json](https://dsh-plugins.omniskill.online/catalog.snapshot.json) |
...
웹사이트와 CLI는 비공개 소스로부터 유지 관리됩니다. 이 저장소에는 이들이 사용하는 공개 카탈로그 데이터, 스키마, 정책이 포함되어 있습니다.

## 카탈로그 현황

**3614개 플러그인 병합 완료.** 모든 플러그인은 원 크리에이터의 저장소로부터, 고정된 소스 커밋과 명시적인 출처 표시와 함께, 한 번에 하나씩 개별적으로 검토된 풀 리퀘스트를 통해 등록됩니다.
...
이 스코프 패키지는 `omni-dsh-plugins@1.0.1`으로 배포되어 있으며, 위 명령어가 현재의 표준 호출 방식입니다. 여기에는 설치 스크립트가 호스팅되어 있지 않습니다.
...
| ------------------------------ | ------------------------------------------------------------------- | --------------------------------------- |
| `catalog validate --catalog .` | 카탈로그 YAML, 스키마, 로컬 시맨틱스를 검증                   | 아니요 — 읽기 전용                          |
| `search <query...>`            | 공개 카탈로그 필드를 로컬에서 검색                                | 아니요 — 읽기 전용                          |
| `info <id>`                    | 하나의 공개 카탈로그 항목을 표시                                       | 아니요 — 읽기 전용                          |
...
| `add <id> --profile <name> --allow-code-execution` | 공식 DSH 위임을 통해 설치        | 예 — 명시적인 동의 플래그가 있을 때만   |
...
변경을 일으키는 명령어(`add`, `update`, `remove`)는 `--allow-code-execution`을 전달하지 않는 한 플러그인의 라이프사이클 코드를 절대 실행하지 않습니다. 네이티브 Windows에서는 v1.0.1에서 이러한 변경 작업이 비활성화되어 있습니다. WSL을 사용하세요. 읽기 전용 및 드라이런 명령어는 어디서나 동작합니다.

## 🔍 플러그인이 카탈로그에 등록되는 과정

![플러그인이 카탈로그에 등록되는 과정](../../docs/diagrams/catalog-flow-dark.svg#gh-dark-mode-only)
![플러그인이 카탈로그에 등록되는 과정](../../docs/diagrams/catalog-flow-light.svg#gh-light-mode-only)

1. **하나의 플러그인, 하나의 브랜치, 하나의 풀 리퀘스트.** PR은 `catalog/plugins/` 아래의 정확히 하나의 YAML 파일만 추가하거나 변경합니다.
2. **크리에이터 우선.** 플러그인의 크리에이터나 소유 조직이 연 PR은, 동일한 플러그인에 대한 커뮤니티 큐레이션이나 자동화보다 항상 우선합니다 — [docs/CREDIT.md](../../docs/CREDIT.md)를 참고하세요.
3. **원본 소스로부터의 증거.** 모든 필드는 고정된 40자 커밋 시점의 크리에이터 저장소로부터 재구성됩니다: 설명, 라이선스, DSH 통합, 설치 디스크립터, 스타 수.
4. **로컬 검증.** `catalog validate`는 구조와 로컬 시맨틱스를 검사합니다. 이는 CI 작업 `catalog-validation`이 PR에서 실행하는 것과 동일한 검사입니다.
5. **메인테이너 게이트.** 병합 전에, 메인테이너는 저장소 신원, 크리에이터와의 연결, 고정된 증거를 별도로 검증합니다. 로컬 검증이 초록불인 것은 필요조건이지만, 결코 충분조건은 아닙니다.

필요한 증거, YAML 규칙, 스타 정책, 충돌 처리, 검토 게이트를 포함한 완전한 계약 내용은 [CONTRIBUTING.md](../../CONTRIBUTING.md)에 있습니다. 결정이 어떻게, 누구에 의해 내려지는지는 [docs/GOVERNANCE.md](../../docs/GOVERNANCE.md)에 있습니다.

## 📄 항목의 구조

각 항목은 자신의 ID에 따라 이름이 붙은 하나의 YAML 파일입니다. 아래 예시는 현재 스키마에 대해 검증됩니다(필드별 참조는 [docs/SCHEMA.md](../../docs/SCHEMA.md)):
...
- `unofficial: true`와 `schemaVersion: 1`은 상수입니다.
- 모노레포 플러그인은 반드시 `stars: null`을 사용해야 합니다 — 부모 프로젝트의 스타는 절대 상속되지 않습니다.
- 설치 디스크립터는 정확한 버전의 npm 패키지이거나 고정된 소스 그 자체 중 하나입니다. 이는 데이터이며, 결코 셸 명령어가 아닙니다.
- `verified` 상태는 검토 가능한 스모크 테스트 증거를 필요로 합니다. 그렇지 않으면 항목은 `smokeTest: null`인 `eligible` 상태가 됩니다.

## 🗂 여기에 포함되는 것

이 저장소는 DeepSeek Harness (DSH)를 위해 독립적으로 배포된 통합을 카탈로그화합니다. 네이티브 플러그인, 플러그인 패밀리, 테마, skill, 클라이언트, 브릿지를 포함합니다. 아티팩트 종류, 기능 카테고리, 인터페이스 태그는 [docs/CATEGORIES.md](../../docs/CATEGORIES.md)에 정의되어 있습니다.

공개되는 각 레코드는 `catalog/plugins/` 아래의 하나의 YAML 파일이며, `schemas/plugin.schema.yaml`에 대해 검증되어야 합니다. 등록되어 있다는 것은 문서화된 적격성 또는 검증 검사가 완료되었음을 의미합니다. 이는 보안 인증도 아니고 DeepSeek의 보증도 아닙니다.

## 🏅 순위와 검증

해당 저장소 고유의 스타를 가진, 전용(dedicated), 네이티브, 적격(eligible), 또는 검증됨(verified) 상태의 플러그인 저장소만이 스타 순위에 포함될 수 있습니다. 더 큰 모노레포 안에 저장된 통합은 계속 검색 가능하지만 `stars: null`을 사용하며, 부모 프로젝트의 스타를 절대 상속하지 않습니다. 전체 판정 기준은 [docs/RANKING.md](../../docs/RANKING.md)를 참고하세요.

공개된 검증 상태는 구조적 적격성과 설치 스모크 테스트를 구분합니다. 어떤 상태도 절대적인 안전을 나타내지 않습니다. 사용하기 전에 플러그인의 저장소, 고정된 커밋, 라이선스, 설치 동작을 검토하세요.

## 🤝 기여하거나 항목을 클레임하기

풀 리퀘스트를 열기 전에 [CONTRIBUTING.md](../../CONTRIBUTING.md)를 읽어보세요. 풀 리퀘스트는 정확히 하나의 플러그인 항목을 추가하거나 변경해야 하며, 다른 카탈로그가 아닌 원 크리에이터의 저장소를 인용해야 합니다. 크리에이터가 직접 작성한 풀 리퀘스트는 자동화된 카탈로그 풀 리퀘스트보다 우선합니다.

크리에이터 클레임, 정정, 삭제를 위한 구조화된 이슈 양식이 제공됩니다. 자격 증명, 개인 연락처, 그 밖의 비밀 정보는 절대 제출하지 마세요.

## 👩‍🎨 플러그인 크리에이터

이 카탈로그는 이 크리에이터들이 플러그인을 배포했기 때문에 존재합니다. 모든 항목은 크리에이터에게 공로를 인정하고 그들의 저장소로 연결됩니다 — 언제나.
...
당신의 플러그인을 온전한 크레딧과 함께 여기에 올리고 싶으신가요? [하나의 YAML 항목으로 하나의 PR을 여세요](../../CONTRIBUTING.md) — 또는 누군가 당신보다 먼저 당신의 작업을 카탈로그화했다면 [기존 항목을 클레임하세요](https://github.com/diegosouzapw/awesome-omni-dsh-plugins/issues/new/choose).
...
| [docs/CLI.md](../../docs/CLI.md)                   | `omni-dsh-plugins@1.0.1`의 CLI 명령어 참조          |
...
이 README는 [`docs/i18n/`](..) 아래에 43개 언어로 제공됩니다 — 상단의 국기 선택기를 사용하세요. 영어가 신뢰할 수 있는 원천이며, 번역과 영어 텍스트가 일치하지 않을 경우 영어 텍스트가 우선합니다. 어떤 번역에 대한 수정이든 일반적인 풀 리퀘스트를 통해 환영합니다.

## 📜 라이선스와 출처 표시

문서와 저장소 템플릿은 [MIT 라이선스](../../LICENSE) 하에 라이선스됩니다. 원본 카탈로그 사실과 편집상의 YAML 메타데이터는 [CC0-1.0](../../LICENSE-CATALOG) 하에 제공됩니다. 업스트림 코드, 이름, 로고, 스크린샷은 원래의 소유자와 라이선스 하에 그대로 유지됩니다. [docs/CREDIT.md](../../docs/CREDIT.md)와 [docs/UNOFFICIAL.md](../../docs/UNOFFICIAL.md)를 참고하세요.

<div align="center">

### ⭐ 이 카탈로그가 플러그인을 찾는 데 도움이 되었다면, 저장소에 스타를 눌러주세요 — 크리에이터가 발견되는 데 도움이 됩니다.

**[웹사이트에서 모든 플러그인 둘러보기 →](https://dsh-plugins.omniskill.online)**
